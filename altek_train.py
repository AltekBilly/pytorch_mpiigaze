...
from gaze_estimation.utils import (AverageMeter, compute_angle_error,
                                   create_train_output_dir, load_config_,
...
def validate(epoch, model, loss_function, val_loader, config, tensorboard_writer, logger)-> float:
...
def main(config: str = None):
    ...
    for epoch in range(1, config.scheduler.epochs + 1):
        ...
        if epoch % config.train.val_period == 0:
            avg_angle_error = validate(epoch, model, loss_function, val_loader, config, tensorboard_writer, logger)

        if (epoch % config.train.checkpoint_period == 0 or epoch == config.scheduler.epochs):
            if best_avg_angle_error is None:
                best_avg_angle_error = avg_angle_error
            elif best_avg_angle_error <= avg_angle_error or np.isnan(avg_angle_error):
                continue
            else:
                best_avg_angle_error = avg_angle_error
                checkpoint_config = {'epoch': epoch, 'config': config.as_dict()}
                checkpointer.save(f'checkpoint_{epoch:04d}_{avg_angle_error:.2f}', **checkpoint_config)

    tensorboard_writer.close()


if __name__ == '__main__':
    # python -u train.py --config configs/mpiigaze/resnet_preact_train.yaml
    config = 'configs/mpiigaze/resnet_preact_train_altek.yaml'
    main(config)
